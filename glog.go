...
func (l *loggingT) formatHeader(s severity, file string, line int) *buffer {
	return l.formatHeaderTime(s, file, line, timeNow())
}

func (l *loggingT) formatHeaderTime(s severity, file string, line int, now time.Time) *buffer {
	if line < 0 {
...
func (l *loggingT) printe(s severity, err *err.Error) {
	...
	for i := 0; i < d; i++ {
		...
		buf := l.formatHeaderTime(s, file, line, time)
		fmt.Fprintf(buf, "%s", err.Msg(i))
		if buf.Bytes()[buf.Len()-1] != '\n' {
			buf.WriteByte('\n')
		}
		l.output(s, buf, file, line, false)
